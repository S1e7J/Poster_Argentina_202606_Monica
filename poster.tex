...
\newlength{\fullwidth}           % <-- Definida aquí (NO dentro del frame)
...
\newenvironment{labelblock}[2]{%
  ...
  \newcommand{\separatorcolumn}{\begin{column}{\sepwidth}\end{column}}
    ...
    \title{Resilient food solutions under abrupt sunlight reduction scenarios: lessons from Argentina}
...
\begin{frame}[t]
  ...
  \begin{columns}[t]
    ...
    \begin{column}{\colwidth}
      \begin{labelblock}{Background}{Motivation}
        Abrupt Sunlight Reduction Scenarios (ASRS) represent low-probability but high-consequence global risks capable of causing severe disruptions to agricultural production. These scenarios may arise from large volcanic eruptions, nuclear conflict, or other events that inject light-blocking particles into the stratosphere. Although volcanic eruptions primarily generate sulfate aerosols and nuclear conflict produces black carbon soot, both mechanisms can reduce surface sunlight and crop yields. Argentina is one of the world's major food-exporting countries, making its capacity to maintain food production under ASRS conditions relevant not only for domestic food security but also for regional and global resilience. This study evaluates how resilient food strategies could mitigate food shortages under abrupt sunlight-reduction scenarios.
      \end{labelblock}
    \end{column}
    ...
    \begin{column}{\colwidth}
      \begin{labelblock}{Research aim}{Objective}
        To evaluate the effectiveness of a phased portfolio of adaptive food-system interventions on per-capita caloric availability in Argentina across a seven-year post-eruption window, modeled under ASRS conditions spanning $5$ to $150$ Tg$^{1}$
    ...
    \begin{column}{\widecolwidth}
      \begin{labelblock}{Methods}{Methodology}
        ...
        \begin{minipage}[t]{0.48\linewidth}
          ...
          \vspace{3pt}
          Crop yield loss estimates drawn from Xia et al.\ (2022)$^{1}$\footnotetext{\footnotesize $^1$ Yield loss estimates are drawn from nuclear ASRS simulations (Xia et al., 2022). Volcanic-specific crop impact models for the Southern Hemisphere were not considered in this study. These figures are used as proxies and represent a key uncertainty in this analysis.}, covering six
      ...
      \begin{labelblock}{Adaptive strategies}{Interventions evaluated}
        ...
        \begin{minipage}[t]{0.22\linewidth}
          ...
          \begin{minipage}[t]{\dimexpr\linewidth-6pt\relax}
            ...
            {\footnotesize A separate sensitivity analysis suggests that expansion up to 108 Mha (beyond the 72 Mha assumed in Scenario 2d) could theoretically sustain 6–22× Argentina's population.}
          \end{minipage}
        \end{minipage}
  ...
  % ==================== CUARTA FILA (bloque ancho que ocupa 2 columnas + 1 columna independiente) ====================
  \begin{columns}[t]
    \separatorcolumn
    ...
    \begin{column}{\widecolwidth}
      \begin{labelblock}{Results}{Caloric availability by scenario}
        ...
        % ---- Caption informativo al final ----
        \par\vspace{4pt}
        \begingroup
        \footnotesize
        \textbf{1a:} Scenario 1a — Pre-event net production ($\sim 4,800$ kcal/day/person). Represents actual food available for human consumption after accounting for livestock and biofuel use. Already 2× the minimum requirement.\\
        \textbf{1b:} Scenario 1b — Pre-event gross production ($\sim 28,000$ kcal/day/person). Includes all food produced including what goes to animals and biofuels. Illustrates the enormous potential for redirection in a crisis.\\
        \textbf{2a:} Scenario 2a — 150 Tg ASRS, no adaptation ($\sim 1,500$ kcal/day/person). Famine conditions: production falls below the 2,100 minimum. Only 71\% of the population's caloric needs are met. Immediate action is required.\\
        \textbf{2b:} Scenario 2b — Livestock and biofuel redirection ($\sim 7,800$ kcal/day/person). Redirecting grain from cattle and biofuels to human consumption triples availability vs. 2a — the single most impactful intervention.\\
        \textbf{2c:} Scenario 2c — + resilient food solutions ($\sim 10,000$ kcal/day/person). Adding seaweed cultivation, crop relocation, industrial sugars, and SCP protein brings production to $\sim 5\times$ minimum requirements.\\
        \textbf{2d:} Scenario 2d — + intensive greenhouse expansion and area increase ($\sim 14,000$ kcal/day/person). Scaling greenhouses to $2.3M$ ha and cultivated area to 72M ha over 3 years. Surplus sufficient to support $\sim 300M$ regional population.
        \endgroup
      \end{labelblock}
    \end{column}
    ...
    \begin{column}{\colwidth}
      \begin{labelblock}{Discussion}{Conclusions}
        \begin{itemize}
          \item A volcanic ASRS of $150$ Tg would reduce Argentina's net food production to $\sim 70\%$ of minimum population requirements - famine without intervention, even with rationing alone.
  ...
  % ==================== QUINTA FILA (tres columnas normales) ====================
  \begin{columns}[t]
    \separatorcolumn
